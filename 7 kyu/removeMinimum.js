...
// The museum of incredible dull things wants to get rid of some exhibitions. Miriam, the interior architect, comes up with a plan to remove the most boring
...
function removeSmallest(numbers) {
  let newArr = [...numbers];
  let smallestRating = Math.min(...newArr);
  let indexOfSmallestRating = newArr.indexOf(smallestRating);

  if (newArr.length === 1) {
    newArr = [];
  } else {
    newArr.splice(indexOfSmallestRating, 1);
  }
  return newArr;
}
...
// removeSmallest([5]);
removeSmallest([211, 180, 307, 249, 127, 138, 223, 292, 380, 341, 308, 127]);
